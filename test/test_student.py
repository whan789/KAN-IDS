...
from model.Student import Student_model
...
def get_args():
    ...
    # --- 경로 설정 ---
    parser.add_argument('--model_dir', type=str, default='/home/anom/junyeong/KAN_transformer/ckpt_final/checkpoints_student_linear_kan', help='Directory containing the trained teacher model checkpoints')
    parser.add_argument('--npz_path', type=str, default='/data/anom/final_data/DL_data_final/final_test_data_merged.npz', help='Path to the test .npz file')
    parser.add_argument('--log_path', type=str, default='./ckpt_final/test_student_kan_lineartransformer.log', help='Path to save the evaluation log file')
...
def evaluate_full_flow_only(model, test_loader, device, num_classes):
    ...
    with torch.no_grad():
        for data, time_info, labels in tqdm(test_loader, desc="Full-Flow Only Evaluation"):
            ...
            logits, _, _ = model(data, time_info=time_info, src_mask=padding_mask)
...
def evaluate_checkpoint(args, DEVICE, model_path, test_loader):
    ...
    # 모델 로드
    model = Student_model(
        input_feature_dim=args.input_dim, num_classes=args.num_classes, seq_length=args.seq_length,
        dropout=args.dropout, architecture=args.student_architecture,
        d_model=args.student_d_model, nhead=args.student_nhead, num_encoder_layers=args.student_layers,
        dim_feedforward=args.student_ffn_dim, use_linear_attention=args.use_linear_attention,
        use_kan=args.use_kan_ffn, mlp_kan_hidden_dim=args.mlp_kan_hidden_dim,
    ...
    flops = FlopCountAnalysis(model, (dummy_input, dummy_time_info, None))
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.end_epoch + 1, args.epoch_step):
        checkpoint_name = f"student_epoch_{epoch}.pth"
